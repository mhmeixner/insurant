...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme845
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme845_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme845_html_head_alter(&$head_elements) {
 ...
 */
function theme845_preprocess_search_block_form(&$vars) {
 ...
 */
function theme845_preprocess(&$vars, $hook) {
  $vars['theme845_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme845_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme845_doctype();
  $vars['rdf'] = _theme845_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme845_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme845_id_safe('section-' . $section);
 ...
 */
function theme845_preprocess_page(&$vars) {
 ...
 */
function theme845_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme845') . '/css/maintenance-page.css');
}
 ...
 */
function theme845_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme845_preprocess_block(&$vars, $hook) {
 ...
 */
function theme845_menu_tree($vars) {
 ...
 */
function theme845_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme845_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme845_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme845_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme845_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme845_breadcrumb_separator');
 ...
 * @return bool
 */
function theme845_tabs_float() {
  $float = (bool) theme_get_setting('theme845_tabs_float');
  $float_node = (bool) theme_get_setting('theme845_tabs_node');
...
function theme845_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme845_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme845_id_safe($element['#title']);
 ...
 */
function theme845_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme845_menu_local_tasks(&$vars) {
  $output = '';
